...
class SettingsController: UIViewController, UITableViewDelegate, UITableViewDataSource, MFMailComposeViewControllerDelegate {
    ...
    func updateSettingsArr(notif: Notification) {
        if let userInfo = notif.userInfo as? [String: AnyObject], let option = userInfo["option"] as? String, let isOn = userInfo["isOn"] as? Bool {
            ...
            DataService.ds.updateUserSettings(dailyAlert: settingsChosen["Daily alert"]!, inboxMessages: settingsChosen["Private messages"]!, playgroupMessages: settingsChosen["Group messages"]!, inAppNotifications: settingsChosen["In-app notifications"]!, completed: { (isSuccess) in
                if !isSuccess {
    ...
    fileprivate func getUserSettings() {
        ...
        self.settingsChosen = ["Daily alert": true, "Private messages": true, "Group messages": true, "In-app notifications": true]
    ...
    fileprivate var settings: [[Setting]] = [
        [Setting(text: "Daily alert", type: .toggle, isOn: true),  Setting(text: "Private messages", type: .toggle, isOn: true), Setting(text: "Group messages", type: .toggle, isOn: true), Setting(text: "In-app notifications", type: .toggle, isLastCell: true, isOn: true)],
        ...
        [Setting(text: "Version 1.1", type: .nobg, isLastCell: true)]]
